...
Issues #2 through #12 have landed, and #15 with them: D1 persistence, the legal text capture, a working sign path, the bilingual content module, the trust pipeline, the live counter, the voivodeship map, the full legal layer, the public supporters list, the full page anatomy, the `init-project` identity interview, and the live supporters list beside a counter that says how long ago the last signature arrived.
...
- **Live counter** — `src/live/live-counter.ts` is the `LiveCounter` Durable Object: one per deployment, addressed through `liveCounter(env)`, upgraded to over `GET /api/live`, accepting sockets with `ctx.acceptWebSocket` so it can hibernate under them. Its notification carries no number — it re-reads D1 and broadcasts, coalesced to ~1/sec through its own alarm, with that alarm doubling as a 30s reconciliation heartbeat while anybody is connected. What it broadcasts is a `LiveUpdate` (`src/core/live-update.ts`): the counts, plus at most `LIVE_SUPPORTERS` of the newest published names. Both halves come from one read, `readLiveUpdate`, which `/api/signatures/snapshot` also answers with — so the socket and its polling fallback cannot carry different things. The object remembers the ids its last push carried and repeats none of them, which is what keeps the heartbeat on a quiet petition costing what it did before; a greeting is unfiltered, because a name published between a page's render and its socket opening would otherwise reach nobody. The page consumes it through `useLiveCounts`, which degrades to polling in silence and returns `{ counts, arrivals }` to the four things that render it: `CounterSection`, `FloatingBar` once the hero is off screen, the map, and the supporters list.
- **The petition's tempo** — `readSignatureCounts` returns `secondsSinceLastSignature` beside the counts, from the same `GROUP BY`: `unixepoch() - max(created_at)`, over every signature whether or not its signer agreed to be named. A duration rather than an instant, because an instant has to be subtracted from the reader's clock and a wrong clock would report a signature from a minute ago as an hour old — or place it in the future. It is also what makes `LastSignature` safe to server-render: the first client render subtracts nothing, so it cannot disagree with the markup it hydrates. The component counts on from there with its own stopwatch, resets on every push, and asks `Intl.RelativeTimeFormat` for the words — so Polish's declined units need no table beside `counter.nouns`. The content files own the label and nothing else. It is not in the floating bar, which has no room for it at 375 px.
...
- Remaining issue (#13) defines the build order. Issue #15 has landed, reversing the plan's D6: the list updates itself from the same push as the counter, and a signature whose signer declined publication moves the counter and leaves the list untouched — which is the intended behaviour rather than a contradiction.
